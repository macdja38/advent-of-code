import fs from "fs";

const example = `
mask = 000000000000000000000000000000X1001X
mem[42] = 100
mask = 00000000000000000000000000000000X0XX
mem[26] = 1
`

const data = fs.readFileSync("./data.txt", { encoding: "utf8" })
let lines = data.split("\n").map(line => line.trim()).filter(line => line !== "");

const TYPE = {
  MASK: 0,
  SET: 1,
}

const instructions = [];

for (let line of lines) {
  if (line.startsWith("mask")) {
    instructions.push({type: TYPE.MASK, value: line.split("=")[1].slice(1) })
  }
  if (line.startsWith("mem")) {
    const [a, b] = line.split("=");
    const address = parseInt(a.slice(4).split("]")[0], 10);
    const v = parseInt(b.slice(1), 10).toString(2);
    instructions.push({type: TYPE.SET, address, value: v });
  }
}

const memory = [];

function applyMask(mask, value) {
  let v = [value.toString(2).padStart(mask.length, "0").split("")];

  for (let i = 0; i < mask.length; i++) {
    switch(mask[i]) {
      case "0":
        break;
      case "1":
        for (let a of v) {
          a[i] = "1"
        }
        break;
      case "X":
        for (let j = 0; j < v.length; j++) {
          let addr = JSON.parse(JSON.stringify([v[j], v[j]]));
          addr[0][i] = "1";
          addr[1][i] = "0";
          v[j] = addr;
        }
        v = v.flat()
    }
  }

  return v.map(v => v.join(""))
}

console.log(instructions)

let mask = "";

for (let instruction of instructions) {
  // console.log(instruction);

  switch (instruction.type) {
    case TYPE.MASK:
      mask = instruction.value;
      break;
    case TYPE.SET:
      const newAdds = applyMask(mask, instruction.address);

      // console.log(newAdds);

      for (let a of newAdds) {
        memory[a] = instruction.value
      }
  }
}

let acc = 0;
for (let v of Object.values(memory)) {
  // console.log(v);
  // console.log(parseInt(v, 2));
  acc += parseInt(v, 2);
}

// console.log(memory);
console.log(acc);
